...
class DatasetInfo(BaseModel):
    ...
    @classmethod
    def from_info(cls, info, path: pathlib.Path):
...
the_file = H5_FILES[0]
the_dataset_info: DatasetInfo = DatasetInfo(path=pathlib.Path(the_file))
...
def deps():
    ...
    while True:
        try:
            ...
            the_dataset_info = DatasetInfo.from_info(
                MIC_SERVER.get_dataset_info(), the_file
            )
...
def afm_microscope(
    ...
    meta = the_dataset_info.model_dump()
